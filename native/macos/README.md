...
Small, Medium, and Large use a fixed header/content/footer scaffold. Header owns the brand and period controls, content owns only the selected page body, and footer owns the page cycling button plus the open-page link. The scaffold uses a single `VStack` with fixed header/footer heights and a flexible content slot, while each page body adapts inside that slot with `ViewThatFits`; keep header/footer outside individual page views so the brand, period controls, page button, and arrow do not move while users cycle pages or periods.
