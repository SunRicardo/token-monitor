...
- `mimo-credentials/<hash>.cookie`（v0.25.0 新增，MiMo 多账号凭据）
...
- `mimoManagedAccounts`（v0.25.0 新增，MiMo 多账号列表，cookie 存独立文件）
- `ollamaSessionCookie`（v0.25.0 新增，Ollama Cloud session cookie）
...
- MiMo：`userData/mimo-credentials/<sha256(id)>.cookie`（v0.25.0 新增，每个账号独立文件）。
...
4. v0.25.0 新增的 MiMo 和 Ollama 都是 web cookie 型 provider，凭据存储方式与传统 API key 型不同。
